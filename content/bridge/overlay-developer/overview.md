...
Ready-made OBS overlays (**Overview**, **Compact**, **Bar**, and others) are **served by Bridge** on your PC — copy URLs from **Printers → Details**. User setup: [Overlays in OBS](../../user/overlays/). This section is for building your own clients or understanding the API those widgets use.
...
2. Either use a built-in overlay (`http://127.0.0.1:29067/bridge/overview/?printer=…` or `/bridge/bar/?printer=…`), [drop your own HTML into Bridge’s custom folder](../custom/), or call the [HTTP & WebSocket API](../api/) from a page you host elsewhere.
...
Streamer.bot is a **separate, optional** path: Bridge can push `DoAction` into Streamer.bot for automation. Overlays do **not** require Streamer.bot — they should use the Bridge API directly unless you intentionally route everything through Streamer.bot.
